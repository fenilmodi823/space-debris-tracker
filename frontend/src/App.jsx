...
function App() {
  ...
  const satRecords = useMemo(() => {
    return satellites.filter(s => s.line1 && s.line2).map(sat => ({
      ...sat,
  ...
  return (
    <>
      ...
      <div style={{
        ...
        backgroundColor: 'rgba(10,10,15,0.85)', padding: '15px 25px', borderRadius: '8px',
        border: '1px solid #333', color: 'white', fontFamily: 'sans-serif',
        boxShadow: '0 4px 15px rgba(0,0,0,0.5)', pointerEvents: 'auto'
      }}>
        <h1 style={{ margin: 0, fontSize: '24px', fontWeight: 'bold', color: '#00ffcc' }}>Space Debris Tracker</h1>
        <p style={{ margin: '5px 0 0', opacity: 0.8, fontSize: '14px', letterSpacing: '1px', textTransform: 'uppercase' }}>Cesium WebGL Engine</p>
      </div>

      {/* Selected Satellite Info Card - UNDER TITLE */}
      {selectedSat && (
        <div style={{
          position: 'absolute', top: 110, left: 20, zIndex: 10,
          backgroundColor: 'rgba(10,10,15,0.9)', padding: '15px 25px', borderRadius: '8px',
          color: 'white', minWidth: '220px', borderLeft: '4px solid #00ffcc',
          fontFamily: 'sans-serif', borderTop: '1px solid #333', borderRight: '1px solid #333', borderBottom: '1px solid #333',
          boxShadow: '0 4px 15px rgba(0,0,0,0.5)', pointerEvents: 'auto'
        }}>
          <h2 style={{ margin: 0, fontSize: '18px', color: '#fff' }}>{selectedSat.name}</h2>
          <div style={{ margin: '12px 0', fontSize: '12px', opacity: 0.7, fontFamily: 'monospace' }}>
      ...
      <div style={{
        ...
        backgroundColor: 'rgba(10,10,15,0.85)', padding: '15px', borderRadius: '8px',
        display: 'flex', flexDirection: 'column', gap: '8px', fontFamily: 'sans-serif',
        border: '1px solid #333', boxShadow: '0 4px 15px rgba(0,0,0,0.5)', pointerEvents: 'auto'
      }}>
        <h3 style={{ margin: '0 0 10px 0', color: '#fff', fontSize: '14px', textAlign: 'center', letterSpacing: '1px', textTransform: 'uppercase' }}>Orbital Shells</h3>
        <button style={filterBtnStyle(showLEO)} onClick={() => setShowLEO(!showLEO)}>
          LEO (&lt; 2000km)
        </button>
        <button style={filterBtnStyle(showMEO)} onClick={() => setShowMEO(!showMEO)}>
          MEO (2000-35786km)
        </button>
        <button style={filterBtnStyle(showGEO)} onClick={() => setShowGEO(!showGEO)}>
          GEO (&gt; 35786km)
        </button>
      </div>
      ...
      >
        ...
        {showLEO && (
          <Entity position={Cartesian3.ZERO}>
            <EllipsoidGraphics 
              radii={new Cartesian3(6371000 + 2000000, 6371000 + 2000000, 6371000 + 2000000)}
        ...
        {showMEO && (
          <Entity position={Cartesian3.ZERO}>
            <EllipsoidGraphics 
              radii={new Cartesian3(6371000 + 20000000, 6371000 + 20000000, 6371000 + 20000000)}
        ...
        {showGEO && (
          <Entity position={Cartesian3.ZERO}>
            <EllipsoidGraphics 
              radii={new Cartesian3(6371000 + 35786000, 6371000 + 35786000, 6371000 + 35786000)}
        ...
        {satRecords.map((sat) => {
          ...
          const positionProperty = new CallbackProperty((time) => {
            ...
            if (posVel.position && posVel.position !== true) {
              ...
              if (isNaN(lat) || isNaN(lon) || isNaN(alt)) return Cartesian3.ZERO;
          ...
          if (isSelected) {
            pathProperty = new CallbackProperty((time) => {
              ...
              for (let i = 0; i <= 120; i++) {
                ...
                const simDate = new Date(JulianDate.toDate(time).getTime() + offset * 60000);
          ...
          return (
            ...
            >
              <PointGraphics 
                ...
                distanceDisplayCondition={new DistanceDisplayCondition(0.0, 45000000.0)}
              />
              <LabelGraphics 
                text={sat.name} 
                ...
                distanceDisplayCondition={new DistanceDisplayCondition(0.0, 25000000.0)}
              />
