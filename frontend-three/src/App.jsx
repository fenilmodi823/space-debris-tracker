...
const IRIDIUM_33_TLE = {
  ...
  line1: "1 24946U 97051C   09040.85227702  .00000185  00000-0  45155-4 0  4245",
  line2: "2 24946  86.3986 244.4714 0002167 259.0763 101.0069 14.34215865593881"
};
const COSMOS_2251_TLE = {
  name: "COSMOS 2251 (DERELICT)",
  line1: "1 22675U 93036A   09041.01815132  .00000067  00000-0  21183-4 0  9154",
  line2: "2 22675  74.0483 283.4385 0015509 255.4859 104.4770 14.28313437775569"
};
  ...
  return (
    <group ref={groupRef}>
      ...
      <mesh>
        ...
        <meshStandardMaterial map={earthTexture} roughness={0.9} metalness={0.15} />
      </mesh>
      {/* Clouds Layer */}
      <mesh>
  ...
  return (
    <group ref={groupRef}>
      ...
      <mesh>
        ...
        <meshBasicMaterial color="#ff0000" transparent opacity={0.15} side={THREE.DoubleSide} blending={THREE.AdditiveBlending} depthWrite={false} />
      </mesh>
      {/* MEO (Medium Risk - Yellow) */}
      <mesh>
        <sphereGeometry args={[20.0, 64, 64]} />
        <meshBasicMaterial color="#ffaa00" transparent opacity={0.08} side={THREE.DoubleSide} blending={THREE.AdditiveBlending} depthWrite={false} />
      </mesh>
      {/* GEO (Low Risk - Green) */}
      <mesh>
        <sphereGeometry args={[42.0, 64, 64]} />
        <meshBasicMaterial color="#00ffaa" transparent opacity={0.05} side={THREE.DoubleSide} blending={THREE.AdditiveBlending} depthWrite={false} />
      </mesh>
    </group>
  );
}
...
function DebrisSwarm() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      object3D.rotation.set(Math.random() * Math.PI, Math.random() * Math.PI, Math.random() * Math.PI);
...
function CameraController({ selectedSat, offsetMinutes, resetTrigger, baseTime }) {
...
function SatelliteMesh({ sat, offsetMinutes, selectedSat, setSelectedSat, baseTime }) {
  const groupRef = useRef();
  const textRef = useRef();
  const [hovered, setHovered] = useState(false);
  
  // Memoize parsing the satellite string to avoid performance issues
  const satrec = useMemo(() => satellite.twoline2satrec(sat.line1, sat.line2), [sat.line1, sat.line2]);
  ...
  return (
    <>
      ...
      {isSelected && orbitPoints.length > 0 && (
        <Line points={orbitPoints} color="cyan" lineWidth={1} transparent opacity={0.5} />
      )}
    </>
  );
}
...
function CollisionEvent({ target, offsetMinutes, baseTime }) {
  ...
  const covRef = useRef();
  const satrec = useMemo(() => satellite.twoline2satrec(target.line1, target.line2), [target]);
  ...
  useFrame(({ clock }) => {
    ...
    if (posVel.position && posVel.position !== true) {
      const pos = posVel.position;
      const scale = 10 / 6371.0;
      const currentPos = new THREE.Vector3(pos.x * scale, pos.z * scale, -pos.y * scale);
...
function SunLight({ offsetMinutes, baseTime }) {
  ...
  useFrame(() => {
    if (sunLightRef.current) {
      const simulatedDate = new Date(baseTime + offsetMinutes * 60000);
      const hours = simulatedDate.getUTCHours() + simulatedDate.getUTCMinutes() / 60;
      const angle = (hours / 24) * Math.PI * 2 + Math.PI; 
      sunLightRef.current.position.set(Math.cos(angle) * 50, 0, Math.sin(angle) * 50);
    }
  });
...
function App() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (posVel.position && posVel.velocity && posVel.position !== true) {
        ...
        setTelemetry({ lon, lat, height, velocity: v, inclination: inc, period });
      }
    }, 500);
  ...
  useEffect(() => {
    ...
    if (isSimulating) {
      ...
      timer = setInterval(() => setSimElapsed((Date.now() - start) / 1000), 250);
    }
  ...
  const exportTelemetryCSV = () => {
    ...
    let csvContent = "Time_Offset_Min,UTC_Time,Latitude,Longitude,Altitude_km,Velocity_km_s\n";
    const satrec = satellite.twoline2satrec(selectedSat.line1, selectedSat.line2);
    ...
    link.setAttribute("download", `${selectedSat.name.replace(/\s+/g, '_')}_telemetry.csv`);
  ...
  const customSelectStyles = {
    control: (base) => ({
      ...base, background: 'transparent', borderColor: 'rgba(0, 255, 204, 0.5)',
      color: '#00ffcc', boxShadow: 'none', '&:hover': { borderColor: '#00ffcc' }
    }),
    singleValue: (base) => ({ ...base, color: '#00ffcc' }),
    input: (base) => ({ ...base, color: '#00ffcc' }),
    menu: (base) => ({ ...base, background: 'rgba(10, 10, 15, 0.95)', border: '1px solid #00ffcc' }),
    option: (base, state) => ({
      ...base, backgroundColor: state.isFocused ? 'rgba(0, 255, 204, 0.2)' : 'transparent',
      color: '#00ffcc', cursor: 'pointer'
    })
  };

  const btnStyle = {
    padding: '8px 16px', backgroundColor: 'transparent', color: '#00ffcc', 
    border: '1px solid rgba(0, 255, 204, 0.4)', borderRadius: '8px', cursor: 'pointer', 
    fontWeight: 'bold', transition: '0.2s', fontFamily: 'monospace'
  };

  const filterBtnStyle = (active) => ({
    padding: '10px', backgroundColor: active ? 'rgba(0, 255, 204, 0.2)' : 'transparent', 
    color: active ? '#00ffcc' : '#aaa', border: '1px solid rgba(0, 255, 204, 0.4)', borderRadius: '8px', cursor: 'pointer', 
    fontWeight: 'bold', textTransform: 'uppercase', transition: '0.2s', fontFamily: 'monospace'
  });
  ...
  return (
    <>
      ...
      <div style={{
        ...glassPanelStyle,
        position: 'absolute', top: '20px', left: '20px', zIndex: 100, minWidth: '300px'
      }}>
        <div style={{ fontSize: '16px', fontWeight: 'bold', letterSpacing: '2px', color: '#fff', marginBottom: '15px', borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: '10px' }}>SPACE DEBRIS TRACKING SYSTEM</div>
        <Select 
          options={satellites.map(s => ({ value: s, label: s.name }))}
          onChange={(opt) => setSelectedSat(opt.value)}
          styles={customSelectStyles}
          placeholder="SEARCH SAT_ID..."
          value={selectedSat ? { value: selectedSat, label: selectedSat.name } : null}
        />
        ...
        {selectedSat && (
          <a 
            ...
            target="_blank" 
            rel="noreferrer"
            style={{ display: 'block', marginTop: '12px', color: '#00ffcc', textDecoration: 'none', fontSize: '11px', border: '1px solid rgba(0, 255, 204, 0.4)', padding: '8px', textAlign: 'center', borderRadius: '4px', letterSpacing: '1px', backgroundColor: 'rgba(0, 255, 204, 0.05)' }}
          >
        ...
        <button 
          ...
          style={{
            marginTop: '15px', padding: '12px 24px', backgroundColor: 'rgba(255, 50, 50, 0.1)', 
            color: '#ff4444', border: '1px solid #ff4444', borderRadius: '4px', width: '100%',
            cursor: 'pointer', fontWeight: 'bold', letterSpacing: '1px', textTransform: 'uppercase', transition: 'all 0.3s ease'
          }}
        >
          ⚠️ RUN ML PREDICTION
        </button>
      ...
      <div style={{
        ...
        <h3 style={{ margin: '0 0 5px 0', color: '#00ffcc', fontSize: '12px', textAlign: 'center', letterSpacing: '1px', textTransform: 'uppercase' }}>FILTERS</h3>
        <button style={filterBtnStyle(showLEO)} onClick={() => setShowLEO(!showLEO)} title="Low Earth Orbit">LEO</button>
        <button style={filterBtnStyle(showMEO)} onClick={() => setShowMEO(!showMEO)} title="Medium Earth Orbit">MEO</button>
        <button style={filterBtnStyle(showGEO)} onClick={() => setShowGEO(!showGEO)} title="Geosynchronous">GEO</button>
        <hr style={{ width: '100%', borderColor: 'rgba(0,255,204,0.2)', margin: '5px 0' }} />
        <button 
          onClick={() => setShowControls(!showControls)}
          style={{ ...btnStyle, background: 'rgba(0,0,0,0.5)' }} title="Toggle UI"
        >
          {showControls ? '◧ UI' : '◨ UI'}
        </button>
        <button 
          onClick={() => { setResetTrigger(prev => prev + 1); setIsSimulating(false); setSelectedSat(null); setDemoTarget(null); }}
          style={{ ...btnStyle, background: 'rgba(0,0,0,0.5)' }} title="Reset Camera"
        >
          🌍 RESET
        </button>
        <button 
          ...
          style={{ 
            ...btnStyle, 
            background: showSwarm ? 'rgba(255, 100, 50, 0.2)' : 'rgba(0,0,0,0.5)',
        ...
        <button 
          ...
          style={{ ...btnStyle, background: 'rgba(255, 50, 50, 0.1)', color: '#ff4444', borderColor: '#ff4444' }} title="Load 2009 Historical Collision Data"
        >
          [ ⏮️ 2009 IRIDIUM COLLISION ]
        </button>
        <button 
          onClick={() => setShowHeatmap(!showHeatmap)} 
          style={{ ...btnStyle, marginTop: '10px', width: '100%', borderColor: showHeatmap ? '#ff0000' : 'rgba(255,255,255,0.2)', color: showHeatmap ? '#ff0000' : '#fff' }}
        >
      ...
      {(selectedSat || isSimulating) && (
        <div style={{
          ...
          borderLeft: isSimulating ? '4px solid #ff4444' : '4px solid #00ffcc',
          borderColor: isSimulating ? 'rgba(255, 68, 68, 0.4)' : 'rgba(0, 255, 204, 0.4)',
          minWidth: '360px'
        }}>
          {isSimulating ? (
            <>
              <div style={{ borderBottom: '1px solid #ff4444', paddingBottom: '8px', marginBottom: '12px', color: '#ff4444', fontWeight: 'bold' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '12px', fontFamily: '"JetBrains Mono", monospace' }}>
                ...
                <span style={{ color: '#fff', textAlign: 'right' }}>{demoTarget?.name || 'UNKNOWN'}</span>
                
                <span style={{ color: '#aaa' }}>ROGUE_ID:</span> 
                <span style={{ color: '#ffaaaa', textAlign: 'right' }}>UNTRACKED_DEBRIS_OBJ</span>
                
                <span style={{ color: '#aaa' }}>TCA:</span> 
                <span style={{ color: '#ff4444', fontWeight: 'bold', textShadow: '0 0 5px #ff4444', textAlign: 'right' }}>
                  T- {Math.max(0, 60 - Math.floor(simElapsed % 60))}s
                </span>
                
                <span style={{ color: '#aaa' }}>PROB_OF_COLLISION (Pc):</span> 
                <span style={{ color: '#ff4444', textAlign: 'right' }}>9.34e-2 (93.4%)</span>
                
                <span style={{ color: '#aaa' }}>MISS_DISTANCE:</span> 
                <span style={{ color: '#fff', textAlign: 'right' }}>48.2m ± 3.1m</span>
                
                <span style={{ color: '#aaa' }}>RELATIVE_VELOCITY:</span> 
                <span style={{ color: '#fff', textAlign: 'right' }}>14.6 km/s</span>
                
                <span style={{ color: '#aaa' }}>COVARIANCE_INTERSECT:</span> 
                <span style={{ color: '#00ffcc', textAlign: 'right' }}>TRUE (MA &lt; 2.0)</span>
                
                <span style={{ color: '#aaa' }}>ML_CONFIDENCE:</span> 
                <span style={{ color: '#00ffcc', textAlign: 'right' }}>HIGH (RDR+OPT)</span>
              </div>
              <button 
                onClick={() => { setIsSimulating(false); setDemoTarget(null); setSelectedSat(null); setSimElapsed(0); }} 
                style={{ 
                  width: '100%', marginTop: '15px', padding: '10px', backgroundColor: 'rgba(255, 0, 0, 0.15)', 
                  border: '1px solid #ff4444', color: '#ff4444', textTransform: 'uppercase', letterSpacing: '2px', 
                  cursor: 'pointer', fontWeight: 'bold', fontFamily: '"JetBrains Mono", monospace', transition: '0.2s'
                }}
              >
                [X] ABORT SIMULATION
              </button>
            ...
            <>
              <h2 style={{ margin: '0 0 15px 0', fontSize: '16px', color: '#00ffcc', textTransform: 'uppercase', borderBottom: '1px solid currentColor', paddingBottom: '8px' }}>
                TELEMETRY DATALINK
              </h2>
              
              <div style={{ marginBottom: '15px', color: '#ccc', fontSize: '13px', lineHeight: '1.6' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span>TARGET_ID:</span> <strong>{selectedSat.name}</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span>NORAD_CAT:</span> <strong>{selectedSat.line1.substring(2, 7).trim()}</strong>
                </div>
              </div>
              
              {telemetry && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '12px', fontFamily: '"JetBrains Mono", monospace', marginBottom: '15px' }}>
                  <span style={{ color: '#aaa' }}>LATITUDE:</span> 
                  <span style={{ color: '#00ffcc', textAlign: 'right' }}>{telemetry.lat.toFixed(4)}°</span>
                  
                  <span style={{ color: '#aaa' }}>LONGITUDE:</span> 
                  <span style={{ color: '#00ffcc', textAlign: 'right' }}>{telemetry.lon.toFixed(4)}°</span>
                  
                  <span style={{ color: '#aaa' }}>ALTITUDE:</span> 
                  <span style={{ color: '#fff', textAlign: 'right' }}>{telemetry.height.toFixed(2)} km</span>
                  
                  <span style={{ color: '#aaa' }}>VELOCITY:</span> 
                  <span style={{ color: '#fff', textAlign: 'right' }}>{telemetry.velocity.toFixed(2)} km/s</span>
                  
                  <span style={{ color: '#aaa' }}>INCLINATION:</span> 
                  <span style={{ color: '#00ffcc', textAlign: 'right' }}>{telemetry.inclination.toFixed(2)}°</span>
                  
                  <span style={{ color: '#aaa' }}>ORBITAL PERIOD:</span> 
                  <span style={{ color: '#00ffcc', textAlign: 'right' }}>{telemetry.period.toFixed(1)} min</span>
                </div>
              )}
              
              <div>
                <h3 style={{ margin: '0 0 8px 0', fontSize: '12px', color: '#00ffcc' }}>[ORBITAL CONGESTION INDEX]</h3>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ flex: 1, height: '6px', backgroundColor: 'rgba(0,255,204,0.1)', borderRadius: '3px', overflow: 'hidden' }}>
                    <div style={{ 
                      width: `${(calculateKesslerDensity(selectedSat) / 10) * 100}%`, 
                      height: '100%', 
                      backgroundColor: calculateKesslerDensity(selectedSat) > 7 ? '#ff4444' : calculateKesslerDensity(selectedSat) > 4 ? '#ffaa00' : '#00ffcc' 
                    }} />
                  </div>
                  <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#fff' }}>{calculateKesslerDensity(selectedSat).toFixed(1)}</span>
                </div>
              </div>
              <button 
                onClick={exportTelemetryCSV} 
                style={{ width: '100%', marginTop: '15px', padding: '10px', backgroundColor: 'rgba(0, 255, 204, 0.1)', border: '1px solid #00ffcc', color: '#00ffcc', cursor: 'pointer', fontFamily: 'monospace', fontWeight: 'bold', letterSpacing: '1px', transition: 'all 0.3s' }}
              >
                [↓] EXPORT DATA (CSV)
              </button>
            </>
      ...
      {showControls && (
        <div style={{ 
          ...
          <div style={{ 
            ...glassPanelStyle, 
            pointerEvents: 'auto', display: 'flex', alignItems: 'center', gap: '20px', padding: '10px 30px', borderRadius: '30px'
          }}>
            <span style={{ color: '#fff', fontSize: '14px', fontWeight: 'bold' }}>
              UTC: {new Date(baseTime + offsetMinutes * 60000).toISOString().substring(11, 19)}
            </span>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button style={{...btnStyle, border: 'none'}} onClick={() => setPlaybackRate(-10)} title="Rewind">⏪</button>
              <button style={{...btnStyle, border: 'none'}} onClick={() => setPlaybackRate(0)} title="Pause">⏸</button>
              <button style={{...btnStyle, border: 'none'}} onClick={() => setPlaybackRate(10)} title="Fast Forward">⏩</button>
              <button style={{...btnStyle, border: 'none', color: '#ff4444'}} onClick={() => { setPlaybackRate(0); setOffsetMinutes(0); setBaseTime(Date.now()); }} title="Go Live">◉</button>
            </div>
            <input 
              type="range" min="-1440" max="1440" value={offsetMinutes} 
              onChange={(e) => { setPlaybackRate(0); setOffsetMinutes(parseFloat(e.target.value)); }} 
              style={{ width: '200px', cursor: 'pointer', accentColor: '#00ffcc' }}
            />
          </div>
        </div>
      )}
      ...
      <Canvas camera={{ position: [0, 0, 25], fov: 45 }}>
        ...
        <CameraController selectedSat={selectedSat} offsetMinutes={offsetMinutes} resetTrigger={resetTrigger} baseTime={baseTime} />
        ...
        {showLEO && <mesh><sphereGeometry args={[13.14, 64, 64]} /><meshBasicMaterial wireframe transparent opacity={0.1} color="cyan" /></mesh>}
        {showMEO && <mesh><sphereGeometry args={[41.39, 64, 64]} /><meshBasicMaterial wireframe transparent opacity={0.1} color="orange" /></mesh>}
        {showGEO && <mesh><sphereGeometry args={[66.17, 64, 64]} /><meshBasicMaterial wireframe transparent opacity={0.1} color="magenta" /></mesh>}
        ...
        {isSimulating && demoTarget && (
          <CollisionEvent target={demoTarget} offsetMinutes={offsetMinutes} baseTime={baseTime} />
        )}
      </Canvas>
    </>
  );
